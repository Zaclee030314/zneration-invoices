...
import { invoiceTotals, type InvoiceCategory, type InvoiceWithItems } from "@/lib/types";
...
export default function InvoicesPage() {
  ...
  const rows = useMemo(() => {
    let list = invoices.map((inv) => ({ inv, ...invoiceTotals(inv, inv.invoice_items) }));
    ...
    return list;
  }, [invoices, category, from, to, search, sortField, sortDir]);
  ...
  async function exportCsv() {
    ...
    await downloadFromApi(`/api/invoices/export-csv?${params.toString()}`, "invoices.csv");
  }
    ...
    <AuthGuard>
      <div className="space-y-4">
        <div className="flex items-end justify-between">
          ...
          <Link href="/invoices/new" className="bg-neutral-900 text-white px-4 py-2 rounded text-sm">
            + New Invoice
          </Link>
        </div>
          ...
          <table className="w-full text-sm bg-white border rounded overflow-hidden">
            <thead className="bg-neutral-50 border-b">
              <tr>
                ...
                <Th field="invoice_no" label="Invoice #" sortField={sortField} sortDir={sortDir} onSort={toggleSort} />
            ...
            <tbody>
              {rows.map(({ inv, total }) => (
                <tr key={inv.id} className="border-b last:border-0 hover:bg-neutral-50">
                  ...
                  <td className="p-2">
                    <Link href={`/invoices/${inv.id}`} className="hover:underline font-medium">{inv.invoice_no}</Link>
                  </td>
                  ...
                  <td className="p-2 text-right whitespace-nowrap">
                    <Link href={`/invoices/${inv.id}/edit`} className="text-xs text-neutral-500 hover:underline mr-3">Edit</Link>
                    <button
                    ...
                    <button onClick={() => deleteInvoice(inv.id)} className="text-xs text-red-500 hover:underline">
                      Delete
                    </button>
                  </td>
                </tr>
...
function Th({
  ...
  return (
    <th
      className={`p-2 cursor-pointer select-none text-${align}`}
